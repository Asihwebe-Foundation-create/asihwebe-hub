...
export function EnquiryForm({ type = "contact" }: { type?: "contact" | "involvement" }) {
  ...
  return <form onSubmit={onSubmit} noValidate className="grid gap-5" aria-label={type === "contact" ? "Contact form" : "Get involved form"}>
    ...
    <div className="grid gap-5 sm:grid-cols-2"><FormField label="Full Name" name="fullName" error={field("fullName")}><Input name="fullName" maxLength={100} aria-invalid={!!errors.fullName} aria-describedby={errors.fullName?"fullName-error":undefined}/></FormField><FormField label="Email Address" name="email" error={field("email")}><Input name="email" type="email" maxLength={255} aria-invalid={!!errors.email} aria-describedby={errors.email?"email-error":undefined}/></FormField></div>
    <div className="grid gap-5 sm:grid-cols-2"><FormField label="Phone Number" name="phone" error={field("phone")}><Input name="phone" type="tel" maxLength={30} aria-invalid={!!errors.phone}/></FormField><FormField label="Organisation (optional)" name="organisation"><Input name="organisation" maxLength={150}/></FormField></div>
    <FormField label={type === "contact" ? "Enquiry Type" : "How would you like to help?"} name="enquiryType" error={field("enquiryType")}><select name="enquiryType" defaultValue="" className="form-select" aria-invalid={!!errors.enquiryType}><option value="" disabled>Select an option</option>{options.map(o=><option key={o}>{o}</option>)}</select></FormField>
    <FormField label="Message" name="message" error={field("message")}><Textarea name="message" rows={6} maxLength={2000} aria-invalid={!!errors.message}/></FormField>
